#!/usr/bin/env python3
"""
Incremental ETL Extract Script - Pull data from production databases
Only extracts records with valid person relationships (no orphans)
Appends/updates staging tables via upsert (no truncation)
"""
...
DEFAULT_CONFLICT_COLUMNS = {
    'chemlink_persons': ['id'],
    'chemlink_experiences': ['id'],
    'chemlink_education': ['id'],
    'chemlink_collections': ['id'],
    'chemlink_query_votes': ['id'],
    'chemlink_view_access': ['id'],
    'chemlink_glossary': ['id'],
    'engagement_persons': ['id'],
    'engagement_posts': ['id'],
    'engagement_comments': ['id'],
    'engagement_groups': ['id'],
    'engagement_group_members': ['id'],
    'engagement_mentions': ['id'],
    'kratos_identities': ['id'],
    'kratos_identity_credentials': ['id'],
    'kratos_sessions': ['id'],
    'kratos_session_devices': ['id']
}
...
def load_to_staging(analytics_conn, schema, table_name, columns, rows, conflict_columns=None):
    """Upsert rows into staging table (append/update without truncation)"""
    if not rows:
        log(f"Skipping {schema}.{table_name} (no data to load)", 'WARNING')
        return 0

    if conflict_columns is None:
        conflict_columns = DEFAULT_CONFLICT_COLUMNS.get(table_name)

    if not conflict_columns:
        raise ValueError(f"No conflict columns configured for {schema}.{table_name}")

    log(f"Upserting {len(rows):,} rows into {schema}.{table_name}...", 'INFO')
    start = time.time()

    # Convert JSON/JSONB columns (dict/list) to JSON strings
    fixed_rows = []
    for row in rows:
        row_list = list(row)
    ...
    insert_query = f"""
        ...
        VALUES ({placeholders})
    """
    non_key_columns = [col for col in columns if col not in conflict_columns]
    if non_key_columns:
        set_clause = ', '.join([f"{col}=EXCLUDED.{col}" for col in non_key_columns])
        insert_query += f" ON CONFLICT ({','.join(conflict_columns)}) DO UPDATE SET {set_clause}"
    else:
    ...
    try:
        with analytics_conn.cursor() as cursor:
            ...
            log(f"  Processing in {num_batches} batches of {batch_size} rows...", 'INFO')
            ...
            for i in range(0, len(fixed_rows), batch_size):
                ...
                progress_pct = (total_upserted / len(fixed_rows)) * 100
                log(f"  Batch {batch_num}/{num_batches}: {total_upserted:,}/{len(fixed_rows):,} rows ({progress_pct:.1f}%) - {batch_time:.2f}s", 'PROGRESS')

                if time.time() - last_status >= 5:
                    elapsed = time.time() - start
                    log(f"  ⏳ Still loading {schema}.{table_name}... {total_upserted:,}/{len(fixed_rows):,} rows ({progress_pct:.1f}%) after {elapsed:.1f}s", 'INFO')
                    last_status = time.time()

        elapsed = time.time() - start
        log(f"Successfully upserted {total_upserted:,} rows to {schema}.{table_name} in {elapsed:.2f}s", 'SUCCESS')
        stats['total_rows_loaded'] += total_upserted
        ...
        elapsed = time.time() - start
        error_msg = f"Error upserting into {schema}.{table_name} after {elapsed:.2f}s: {str(e)}"
        log(error_msg, 'ERROR')
        log(f"  Failed at row {total_upserted}/{len(rows)}", 'ERROR')
...
def get_table_columns(source_conn, table_name, schema='public'):
    """Fetch available columns for a source table"""
    query = """
        SELECT column_name
...
def build_dynamic_select_query(source_conn, table_name, column_specs, order_by=None, schema='public'):
    """
    Build a SELECT statement resilient to schema differences.
    column_specs: [{'target': 'col', 'sources': ['col','alt'], 'default': 'NULL::type'}, ...]
    """
...
def extract_kratos_data(analytics_conn):
    """Extract Kratos identity + session data (incremental/upsert)"""
    log("\n" + "="*70)
    ...
    if rows:
        load_to_staging(
            analytics_conn,
            'staging',
            'kratos_identity_credentials',
            columns,
            rows,
            conflict_columns=DEFAULT_CONFLICT_COLUMNS.get('kratos_identity_credentials')
        )
    ...
    for config in KRATOS_TABLE_CONFIG:
        ...
        if rows:
            load_to_staging(
                ...
                columns,
                rows,
                conflict_columns=DEFAULT_CONFLICT_COLUMNS.get(config['staging_table'])
            )
